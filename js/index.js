...
async function convertFiles() {
    ...
    try {
        ...
        for (const file of filesToConvert) {
            ...
            const bmpBlob = await convertToBmp(imgElement);
            convertedFiles.push(bmpBlob);
        }
...
function convertToBmp(imgElement) {
...
function createSingleDownloadLink(blob) {
    ...
    downloadButton.download = `${originalFilename}_prepphint.com.bmp`;
}
...
function createDownloadLink(blobs) {
    ...
    blobs.forEach((blob, index) => {
        ...
        zip.file(`${originalFileName}_prepphint.com.bmp`, blob);
    });
